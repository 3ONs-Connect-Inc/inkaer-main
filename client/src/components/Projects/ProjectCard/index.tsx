      ...
      <Card className="flex flex-col min-h-[360px] h-[420px] sm:h-[460px] md:h-[500px] w-full max-w-sm mx-auto rounded-2xl group shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 overflow-hidden">
        <div className="relative max-h-auto h-48 object-cover  w-full overflow-hidden">
          ...
          <div className="absolute top-4 left-4">
            <span
              className={`px-1 xs:px-2 py-1 text-[10px] rounded-full text-white font-sora capitalize ${getProjectTypeColor(
                type
              )}`}
            >
              {type}
          ...
          <div className="absolute top-4 right-4">
            <span
              className={`px-1 xs:px-2 py-1 text-[10px] xs:text-xs rounded-full font-sora ${getRankColor(
                rank
              )}`}
            >
              {rank}
        ...
        <CardFooter className="p-4 pt-0">
          <Button
            onClick={handleClick}
            className="btn-with-icon w-full bg-gradient-inkaer text-white rounded-full group"
          >
            {getButtonText(type)}
        ...
        <div className="flex gap-6">
          <div className="relative flex-shrink-0 w-48 h-32 overflow-hidden rounded-xl">
            ...
            <div className="absolute top-3 left-3">
              <span
                className={`px-1 xs:px-2 py-1 ${getProjectTypeColor(
                  type
                )} text-white rounded-full text-[10px] font-semibold font-sora capitalize`}
              >
                {type}
              </span>
            </div>
            <div className="absolute top-3 right-3">
              <span
                className={`px-1 xs:px-2 py-1 rounded-full text-[10px] font-semibold font-sora ${getRankColor(
                  rank
                )}`}
              >
                {rank}
          ...
          <div className="flex-1 flex flex-col justify-between">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between mt-4">
              ...
              <Button
                onClick={handleClick}
                className="btn-with-icon bg-blue-500 hover:bg-blue-600 text-white group"
              >
                {getButtonText(type)}
                <ArrowRight className="icon w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
              </Button>
